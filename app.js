...
let getQuote = () => {
  fetch(apiURL)
    ...
    .then((items) => {
      ...
      quoteElement.innerText = item.text;
      let author = item.author.replace(", type.fit", "");
...
let getFortune = () => {
  let fortune = fortuneCookieList[Math.floor(Math.random() * fortuneCookieList.length)];
  fortuneElement.innerText = fortune;
};

window.addEventListener("load", getQuote);
window.addEventListener("load", getFortune);
if(generate) {
